...
//Swap function to sort array of times:
...
//Sort function to sort array of times:
void sort(double list[], unsigned n) {
  unsigned i, j, min;
  for(i = 0; i < n -1; i++) {
    min = i;
    for(j = i + 1; j < n; j++) {
...
//Sum function to find mean throughput:
double sum_array(double list[], unsigned n) {
  double result = 0.0;
  unsigned i;
  for(i = 0; i < n; i++) {
    result += list[i];
...
int main(int argc, char *argv[])
{
  ...
  double latencies[atoi(argv[2])];
  for (int iter = atoi(argv[2]); iter > 0; --iter) {
    clock_t t0 = clock();
    convert_all(nlines, lines, nums);
    double t = (double)(clock() - t0) / CLOCKS_PER_SEC;
    latencies[atoi(argv[2]) - iter] = t;
    assert(checksum(nums, nlines) == QUOTES_CSUM);
    if (t < min_t) {
      min_t = t;
    }
  }
  sort(latencies, atoi(argv[2]));
  unsigned percentile = atoi(argv[2]) * .95;
  double sum = sum_array(latencies, atoi(argv[2]));
  double throughput = (double)(atoi(argv[2])) / sum;
  printf("Best run time overall: %gs\n", min_t);
  printf("95th percentile latency: %gs\n", latencies[percentile]);
  printf("Mean throughput: %g\n", throughput);
  free(nums);
  free(lines);
  free(data);
  return 0;
